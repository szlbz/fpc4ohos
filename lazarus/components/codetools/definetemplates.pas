...
uses
  ...
  // LazUtils
  LazStringUtils, LConvEncoding, LazFileUtils, FileUtil, LazFileCache,
...
const
  ...
  FPCOperatingSystemNames: array[1..8] of shortstring =(
     'linux',
     'win32',
     'win64',
     'embedded',
  ...
  FPCOperatingSystemCaptions: array[1..8] of shortstring =(
     'Linux',
     'Win32',
     'Win64',
     'embedded',
  ...
  FPCProcessorNames: array[1..9] of shortstring =(
      'aarch64',
      'loongarch64',
      'riscv64',
      'x86_64',
      ...
      repeat
        ...
        while i<=OutLen do begin
          if Buf[i] in [#10,#13] then begin
            ...
            Result.Add({$ifdef windows}CP936ToUTF8{$endif}(OutputLine));
            OutputLine:='';
...
function ParseFPCVerbose(List: TStrings; const WorkDir: string; out
  ...
  var
    ...
    p: SizeInt;
  begin
    {$ifdef linux}Line:=SysToUtf8(Line);{$endif}
    len := length(Line);
...
begin
  ...
  or (CompareText(TargetOS,'ohos')=0)
  then
    Result:='unix'
  else
